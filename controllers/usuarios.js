...
module.exports = {
    async listarUsuarios(request, response) {
        try {
            // instruções SQL
            const sql = `SELECT 
            usu_id, usu_nome, usu_email, usu_dt_nasc, usu_senha, usu_tipo, usu_ativo = 1 AS usu_ativo
            FROM usuarios
            WHERE usu_ativo = 1;`;
    ...
    async cadastrarUsuarios(request, response) {
        try {
            ...
            VALUES (?,?,?,?,?,?)`;
            // definição dos dados a serem inseridosem um array
            ...
            return response.status(200).json({
                sucesso: true,
                messagem: 'Cadastro de usuário efetuado com sucesso.',
                dados: usu_id
                //mensSql: execSql
            });
        } catch (error) {
    ...
    async login(request, response) {
        try {
            ...
            const sql = `SELECT usu_id, usu_nome, usu_tipo FROM usuarios WHERE usu_email = ? AND usu_senha=? AND usu_ativo=1;`;
            ...
            return response.status(200).json({
                sucesso: true,
                messagem: 'Login efetuado com sucesso',
                dados: usuarios[0]
            })
